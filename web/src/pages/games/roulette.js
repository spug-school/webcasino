...
export async function Roulette(req) {
  ...
  root.innerHTML = `
      <div class="game-container">
          <h1>Ruletti</h1>
          <p>Tervetuloa pyörittämään (yksinkertaistettua) rulettia! Syötä panos ja arvauksesi!</p>
          <div id="game-area">
              <div class="wrap">
                  <div class="left">
                  <div id="result-area">
          ...
          <form class="wrap-me" id="roulette-form">
              ...
              <label for="number">Numeroarvaus</label>
              <input type="number" id="number" name="number" placeholder="Syötä numero" min="1" max="36">
              <label for="color">Väriarvaus</label>
              <select id="color">
                <option value="p">Punainen</option>
                <option value="m">Musta</option>
              </select>
              <button id="play">Pyöräytä!</button>
          </form>
          </div>
            </div>
        </div>
...
function displayOutcome(outcome) {
  ...
  outcomeText.textContent = `Pyöräytyksen tulos: ${outcome.roll.number}, ${outcome.roll.color_name}. ${outcome.won ? "Voitit!" : "Hävisit..."}`;
